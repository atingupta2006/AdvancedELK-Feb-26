...
# SECURE VARIANT — authenticates via API key or basic auth.
#
# Usage:
#   With basic auth:  bash seed-data-secure.sh <ES_HOST> elastic:<PASSWORD>
#   With API key:     bash seed-data-secure.sh <ES_HOST> apikey:<ENCODED_KEY>
#
# Examples:
#   bash seed-data-secure.sh http://192.168.56.101:9200 elastic:Training123!
#   bash seed-data-secure.sh http://192.168.56.101:9200 apikey:YWJjMTIzOnhlejc4OQ==
# ============================================================
...
AUTH_HEADER=""
AUTH_CURL=""
if [[ "$AUTH_PARAM" == apikey:* ]]; then
    API_KEY="${AUTH_PARAM#apikey:}"
    AUTH_HEADER="-H \"Authorization: ApiKey $API_KEY\""
    AUTH_CURL=(-H "Authorization: ApiKey $API_KEY")
    echo "Auth: API key"
elif [[ "$AUTH_PARAM" == *:* ]]; then
    AUTH_CURL=(-u "$AUTH_PARAM")
    echo "Auth: Basic auth (${AUTH_PARAM%%:*})"
else
    echo "WARNING: No auth provided. If security is enabled, requests will fail."
    echo "Usage: bash seed-data-secure.sh <ES_HOST> elastic:<PASSWORD>"
    AUTH_CURL=()
fi
...
echo "=== Seeding Sample Data (Secure) ==="
echo "Target: $ES_HOST"

# ---- Check existing data ----
WEB_COUNT=$(curl -s "${AUTH_CURL[@]}" "$ES_HOST/web-logs-*/_count" 2>/dev/null | _count || echo "0")
APP_COUNT=$(curl -s "${AUTH_CURL[@]}" "$ES_HOST/app-logs-*/_count" 2>/dev/null | _count || echo "0")
...
if [ "$WEB_COUNT" -gt 0 ] 2>/dev/null && [ "$APP_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    echo "To re-seed, delete and re-run:"
    echo "  curl ${AUTH_CURL[*]} -X DELETE $ES_HOST/web-logs-2026.03.07"
    echo "  curl ${AUTH_CURL[*]} -X DELETE $ES_HOST/app-logs-2026.03.07"
    exit 0
fi
...
curl -s -X POST "${AUTH_CURL[@]}" "$ES_HOST/web-logs-2026.03.07/_bulk" \
...
sleep 1
WEB_RESULT=$(curl -s "${AUTH_CURL[@]}" "$ES_HOST/web-logs-2026.03.07/_count" | _count)
echo "  → $WEB_RESULT documents indexed"

# ---- Seed app-logs (10 docs) ----
echo ""
echo "[2/2] Seeding app-logs-2026.03.07 (10 documents)..."
curl -s -X POST "${AUTH_CURL[@]}" "$ES_HOST/app-logs-2026.03.07/_bulk" \
  -H "Content-Type: application/json" \
  --data-binary @- << 'BULK'
{"index":{}}
...
sleep 1
APP_RESULT=$(curl -s "${AUTH_CURL[@]}" "$ES_HOST/app-logs-2026.03.07/_count" | _count)
echo "  → $APP_RESULT documents indexed"

# ---- Verification ----
echo ""
ENR_COUNT=$(curl -s "${AUTH_CURL[@]}" "$ES_HOST/enriched-logs-*/_count" | _count || echo "0")
